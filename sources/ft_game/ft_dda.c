...
/*   Updated: 2024/04/04 13:45:28 by tlassere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static float	ft_colide(t_ray_data ray, t_data *data)
{
	float	ray_dist;
	int		checker;
...
float	ft_dda(t_data *data)
{
	...
	ray.end.x = ray.start.x + (5 * cos(data->player.rotat * PI180));
	ray.end.y = ray.start.y + (5 * -sin(data->player.rotat * PI180));
